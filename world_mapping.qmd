...
```{r}
world <- ne_countries(scale = "medium", returnclass = "sf") #sf stands for spatial feature
class(world)
```
...
world <- world %>%
  mutate(gdp_bin = cut(
    gdp_md,
    breaks = c(0, 125000, 250000, 500000, 1000000, 5000000, 10000000, Inf), 
    labels = c("<125K", "125K-250K", "250K-500K", "500K-1M", "1M-5M", "5M-10M", ">10M"),  
    include.lowest = TRUE
  ))

world_gdp_bin <- ggplot(data = world, aes(fill = gdp_bin, label = gdp_md)) +
    geom_sf() +
    scale_fill_viridis_d()

ggplotly(world_gdp_bin)
```
