...
percent_completed_no_use_by_state <- teds_d_select %>%
  ...
  summarize(completed_cases = sum(reason == "completed", na.rm = TRUE),
            completed_no_use_cases = sum(reason == "completed" & freq1_d == "no use", 
                                         na.rm = TRUE),) %>%
  mutate(
    percentage_completed_no_use = ifelse(
      completed_cases > 0, (completed_no_use_cases / completed_cases) * 100, 0))
```
...
completed_no_use_service_data <- teds_d_select %>%
  ...
  summarize(total_cases = n(),
            ...
            completed_no_use_cases = sum(reason == "completed" & freq1_d == "no use", 
                                         na.rm = TRUE)) %>%
  mutate(percentage_completed_no_use = (completed_no_use_cases / total_cases) * 100) %>% 
  mutate(percentage_completed = (completed_cases / total_cases) * 100) %>% 
  mutate(percentage_no_use = (no_use_cases / total_cases) * 100)
```

```{r}
ggplot(completed_no_use_service_data, aes(color = services_d, x = percentage_no_use, y = percentage_completed)) +
  geom_point()
```

```{r}
ggplot(completed_no_use_service_data, aes(x = services_d, y = completed_no_use_cases, fill = percentage_completed_no_use)) +
  geom_bar() +
...
```{r}
ggplot(completed_no_use_los_data, aes(color = los, x = percentage_no_use, y = percentage_completed)) +
  geom_point()
```

```{r, warning = FALSE}
ggplot(completed_no_use_los_data, aes(x = los, y = completed_no_use_cases, fill = percentage_completed_no_use)) +
  geom_col()
```
...
```{r, warning = FALSE}
ggplot(completed_no_use_los_services_data, aes(x = los, y = percentage_no_use, color = services_d)) +
  geom_point()

ggplot(completed_no_use_los_services_data, aes(x = los, y = percentage_completed, color = services_d)) +
  geom_point()

ggplot(completed_no_use_los_services_data, aes(x = los, y = percentage_completed_no_use, color = services_d)) +
  geom_point()
```
