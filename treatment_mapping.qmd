...
```{r, warning = FALSE}
completed_no_use_service_los_data <- teds_d_select %>%
  group_by(services_d) %>%
...
```{r}
ggplot(completed_no_use_service_los_data, aes(color = services_d, x = percentage_no_use, y = percentage_completed)) +
  geom_point()
```
