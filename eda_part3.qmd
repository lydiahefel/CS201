...
```{r, message = FALSE}
olympic_data <- read_csv(here("data/athlete_events.csv"))
...
```{r, warning = FALSE}
 ggplot(olympic_data, aes(x = weight, y = height)) +
  geom_point()
...
```{r}
ggplot(olympic_data, aes(x = season, fill = sex)) +
  geom_bar()
```

```{r}
ggplot(olympic_data, aes(x = noc, fill = sex)) +
  geom_bar()
```

```{r}
medal_data <- olympic_data %>% 
  dplyr::filter(!is.na(medal)) %>% 
  ggplot(medal_data, aes(x = medal, fill = sex)) +
  geom_bar()

#medal_data is not found
```
